...
import { adminApi, feedbackApi, notificationApi, type AdminUserView, type FeedbackView, type VerificationView } from "@/lib/api/client";
...
export function AdminModule({ activeId }: Props) {
  ...
  if (["adm-universities", "adm-uni-admins", "adm-route-uni", "adm-audit", "adm-fare"].includes(activeId)) {
    const title =
      activeId === "adm-universities" ? "Trường đại học đối tác"
      : activeId === "adm-uni-admins" ? "Tài khoản Admin trường"
      : activeId === "adm-route-uni" ? "Gán tuyến cho trường"
      : activeId === "adm-fare" ? "Điều chỉnh giá vé"
      : "Audit log";
...
function Unavailable({ title }: { title: string }) {
  return (
    <div>
      <PageHeader title={title} description="UI được giữ lại, nhưng MVP hiện chưa có API production cho phần này." icon={<School className="size-7" />} />
      <UnavailablePanel
        title="Chưa có API production"
        description="Không còn dữ liệu mẫu. Khi backend university/audit/fare APIs được build, màn này sẽ nối vào dữ liệu thật."
      />
    </div>
  );
}
