...
export default function AdminReportsPage() {
  const [reports, setReports] = useState(reportsFromBackend);
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-3">
            {reports.length === 0 && (
              <div className="rounded-2xl border border-dashed border-black/10 bg-brand-surface/40 p-6 text-center text-sm font-bold text-brand-text/50">
                Chưa có dữ liệu khiếu nại từ backend.
              </div>
            )}
            {reports.map((report) => (
              <div
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-black/5 flex flex-col">
              ...
              <div className="flex-1 flex flex-col gap-4 mb-4">
                <div className="bg-brand-surface p-4 rounded-2xl max-w-[80%]">
                  <div className="text-sm font-medium text-brand-text/50">Chưa có ghi chú xử lý từ backend.</div>
                </div>
              </div>
