...
import { UserCircle, Mail, Phone, MapPin, GraduationCap, ShieldCheck, KeyRound, Save } from 'lucide-react';
...
export default function StudentProfilePage() {
  const [formData, setFormData] = useState({
    ...
    address: 'Ký túc xá DMC, Liên Chiểu, Đà Nẵng'
  });
  ...
  return (
    <div className="h-full flex flex-col gap-6 font-sans relative">
      ...
      <div>
        ...
        <p className="text-brand-text/60 font-medium">Quản lý thông tin tài khoản và bảo mật.</p>
      </div>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-black/5 flex flex-col items-center text-center relative overflow-hidden">
            ...
            <div className="w-full flex flex-col gap-3 text-left">
              <div className="flex items-center gap-3 p-3 bg-brand-surface rounded-xl">
                <GraduationCap className="w-5 h-5 text-brand-text/40" />
                <div>
                  <div className="text-xs font-bold text-brand-text/40 uppercase">Trường</div>
                  <div className="font-bold">ĐH Bách Khoa Đà Nẵng</div>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-brand-surface rounded-xl">
                <UserCircle className="w-5 h-5 text-brand-text/40" />
                <div>
                  <div className="text-xs font-bold text-brand-text/40 uppercase">Mã Sinh viên</div>
                  <div className="font-bold">{formData.studentId}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="xl:col-span-2 flex flex-col gap-6">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-black/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-bold text-brand-text/70 mb-2 flex items-center gap-2">
                  <UserCircle className="w-4 h-4" /> Mã sinh viên
                </label>
                <input 
                  type="text" 
                  name="studentId"
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-black/5">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between p-4 border border-black/5 rounded-2xl">
              <div>
                <h4 className="font-bold">Mật khẩu</h4>
