...
const routesFromBackend = [];
const stopsFromBackend = [];

export default function CoordinatorRoutesPage() {
  const [activeRoute, setActiveRoute] = useState(routesFromBackend[0]?.id ?? '');
  const [stops] = useState(stopsFromBackend);
  ...
  const handleSaveStop = (e) => {
    e.preventDefault();
    closeModal();
    setNotice('Chức năng lưu trạm dừng chưa được kết nối với backend.');
  };

  const handleDeleteStop = () => {
    setNotice('Chức năng xóa trạm dừng chưa được kết nối với backend.');
  };

  const activeRouteName = routesFromBackend.find(r => r.id === activeRoute)?.name ?? 'tuyến chưa được chọn';
          ...
          <div className="p-6 md:p-8 border-b border-black/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-brand-surface/30 shrink-0">
            <div>
              ...
              <p className="text-sm font-bold text-brand-text/60">
                Lộ trình chi tiết của {activeRouteName}
              </p>
            </div>
            <button
