...
export default function DriverTripPage() {
  const [currentStationIndex, setCurrentStationIndex] = useState(() => {
    const currentIndex = currentTripMock.stops.findIndex((station) => station.status === 'current');
    return currentIndex >= 0 ? currentIndex : 0;
  });

  const routeStations = currentTripMock.stops;
    ...
    <div className="h-full flex flex-col gap-6 font-sans">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-brand-text/60 font-medium">{currentTripMock.routeName} • {currentTripMock.licensePlate}</p>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-brand-primary rounded-3xl p-6 md:p-8 shadow-sm border border-black/5 flex flex-col justify-center text-center relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 mt-8 relative z-10">
              <Stat label="Tốc độ" value={`${currentTripMock.averageSpeed}`} suffix="km/h" />
              <Stat label="Khách trên xe" value={`${currentTripMock.passengerCount}`} suffix={`/${currentTripMock.capacity}`} />
            </div>
          </div>
            ...
            <div className="flex items-center justify-between p-4 bg-brand-surface rounded-2xl border border-black/5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-secondary/20 rounded-full flex items-center justify-center text-brand-secondary font-bold">
                  {initials(currentTripMock.conductorName)}
                </div>
                <div>
                  <div className="font-bold text-sm">{currentTripMock.conductorName}</div>
                  <div className="text-xs font-medium text-brand-text/60">SĐT: {currentTripMock.conductorPhone}</div>
                </div>
              </div>
              <a href={`tel:${currentTripMock.conductorPhone.replaceAll(' ', '')}`} className="p-3 bg-brand-primary rounded-xl text-brand-text hover:bg-black hover:text-white transition-colors shadow-sm">
