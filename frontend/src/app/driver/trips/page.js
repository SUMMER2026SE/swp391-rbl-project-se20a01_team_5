...
export default function DriverTripPage() {
  ...
  const tripProgress = useMemo(() => {
    ...
    return Math.min(
      100,
      Math.round((currentStationIndex / routeStations.length) * 100),
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-brand-text/60 font-medium">
            {trip?.routeName || "--"} - Biển số: {trip?.licensePlate || "--"}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-black/5 flex flex-col relative overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4 mb-8 relative z-10">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <MapPin className="w-6 h-6 text-brand-primary" /> Lộ trình di
              chuyển
            </h2>
            <div className="text-right">
              <div className="text-xs font-bold text-brand-text/40 uppercase">
                Tiến độ
              </div>
          ...
          <div className="flex-1 relative z-10 pl-4 md:pl-10">
            ...
            {routeStations.map((station, index) => {
              ...
              return (
                <div
                  key={station.id}
                  className="relative z-10 flex items-start gap-6 mb-8 last:mb-0 group"
                >
                  <div
                    className={`
                  ...
                  >
                    <div className="flex justify-between items-center mb-1 gap-4">
                      <h3
                        className={`font-bold text-lg ${isCurrent ? "text-brand-primary" : "text-brand-text"}`}
                      >
                        {station.name}
                      </h3>
                      <span className="font-mono text-sm font-bold text-brand-text/50">
                        {station.time}
                      </span>
                    </div>
                    {isCurrent && (
                      <p className="text-sm font-medium text-brand-text/70">
                        Đang tiến đến trạm này...
                      </p>
                    )}
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-brand-primary rounded-3xl p-6 md:p-8 shadow-sm border border-black/5 flex flex-col justify-center text-center relative overflow-hidden">
            ...
            {!isTripFinished ? (
              ...
              >
                <Flag className="w-6 h-6 text-brand-primary fill-brand-primary" />{" "}
                Đã đến trạm
              </button>
            ) : (
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-black/5">
            <h3 className="font-bold mb-4 flex items-center gap-2">
              <Users className="w-5 h-5 text-brand-secondary" /> Phụ xe chuyến
              này
            </h3>
            ...
            <div className="flex items-center justify-between p-4 bg-brand-surface rounded-2xl border border-black/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-brand-text/60">
                    SĐT: {trip?.conductorPhone || "--"}
                  </div>
                </div>
              </div>
              <a
                href={`tel:${(trip?.conductorPhone || "").replaceAll(" ", "")}`}
                className="p-3 bg-brand-primary rounded-xl text-brand-text hover:bg-black hover:text-white transition-colors shadow-sm"
              >
...
function Stat({ label, value, suffix }) {
  return (
    <div className="bg-white/40 p-4 rounded-2xl border border-white/50">
      <div className="text-xs font-bold text-brand-text/50 uppercase mb-1">
        {label}
      </div>
