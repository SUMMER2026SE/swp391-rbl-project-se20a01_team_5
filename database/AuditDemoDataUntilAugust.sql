    ...
    SELECT
        ...
        CASE
            WHEN count(t.trip_id) >= 2
             AND count(t.trip_id) FILTER (WHERE driver_user.email = 'driver.demo@unibus.local' AND conductor_user.email = 'conductor.demo@unibus.local') = count(t.trip_id)
            THEN 'PASS' ELSE 'FAIL'
        END AS status,
        concat('trips=', count(t.trip_id), '; correctly_assigned=', count(t.trip_id) FILTER (WHERE driver_user.email = 'driver.demo@unibus.local' AND conductor_user.email = 'conductor.demo@unibus.local')) AS detail
    FROM params p
...
ticket_status AS (
    ...
    SELECT 'ticketing', 'day ticket',
           CASE WHEN EXISTS (SELECT 1 FROM single_trip_tickets WHERE student_code = 'SV-DEMO-DAY' AND status = 'UNUSED' AND expires_at >= CURRENT_TIMESTAMP) THEN 'PASS' ELSE 'FAIL' END,
           'SV-DEMO-DAY should have an unexpired UNUSED single ticket'
    UNION ALL
