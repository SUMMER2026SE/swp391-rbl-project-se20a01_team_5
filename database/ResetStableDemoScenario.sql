...
BEGIN
    ...
    INSERT INTO universities (code, name, short_name, contact_email, status, created_at, updated_at)
    VALUES ('STABLE_DEMO_U', 'Đại học Demo UniBus', 'Demo UniBus', 'university.demo@unibus.local', 'ACTIVE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    SELECT university_id INTO v_supported_university_id FROM universities WHERE code = 'STABLE_DEMO_U';
    SELECT university_id INTO v_dtu_university_id FROM universities WHERE name = 'Trường Đại học Duy Tân' ORDER BY university_id LIMIT 1;

    INSERT INTO campuses (university_id, code, name, address, latitude, longitude, status, created_at, updated_at)
    VALUES (v_supported_university_id, 'STABLE_MAIN', 'Cơ sở demo UniBus', 'Đà Nẵng', 16.0544, 108.2022, 'ACTIVE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    FROM (VALUES
        ('SV-STABLE-SUB', 'student.supported@unibus.local', 'Đại học Demo UniBus', 'Công nghệ thông tin', 2023, '2005-03-10', v_supported_university_id),
        ('SV-STABLE-FULL', 'student.fullprice@unibus.local', COALESCE((SELECT name FROM universities WHERE university_id = v_dtu_university_id), 'Trường Đại học Duy Tân'), 'Kinh tế', 2022, '2004-08-12', v_dtu_university_id),
        ('SV-STABLE-MONTH', 'student.monthly@unibus.local', 'Đại học Demo UniBus', 'Kỹ thuật phần mềm', 2021, '2003-04-22', v_supported_university_id),
        ('SV-STABLE-DAY', 'student.day@unibus.local', 'Đại học Demo UniBus', 'Du lịch', 2024, '2006-01-19', v_supported_university_id),
        ('SV-STABLE-UNPAID', 'student.unpaid@unibus.local', 'Đại học Demo UniBus', 'Tài chính', 2023, '2005-05-05', v_supported_university_id),
        ('SV-STABLE-HIST', 'student.history@unibus.local', 'Đại học Demo UniBus', 'Logistics', 2022, '2004-11-30', v_supported_university_id)
    ...
    SELECT route_id INTO v_route_supported_id FROM routes WHERE route_code = 'UB-DN-01' AND status = 'ACTIVE' ORDER BY route_id LIMIT 1;
    IF v_route_supported_id IS NULL THEN
        SELECT route_id INTO v_route_supported_id FROM routes WHERE status = 'ACTIVE' ORDER BY route_id LIMIT 1;
    END IF;
    ...
    WHERE r.status = 'ACTIVE'
      ...
      AND NOT EXISTS (
          ...
          WHERE ru.route_id = r.route_id
            AND ru.university_id = v_dtu_university_id
            AND ru.status = 'ACTIVE'
            AND ru.active_from <= CURRENT_DATE
            AND (ru.active_until IS NULL OR ru.active_until >= CURRENT_DATE)
      )
    ORDER BY CASE WHEN r.route_code = 'UB-DN-04' THEN 0 ELSE 1 END, r.route_id
    LIMIT 1;

    IF v_route_full_id IS NULL THEN
        SELECT route_id INTO v_route_full_id FROM routes WHERE status = 'ACTIVE' AND route_id <> v_route_supported_id ORDER BY route_id LIMIT 1;
    END IF;
    IF v_route_full_id IS NULL THEN
        v_route_full_id := v_route_supported_id;
    END IF;
    IF v_route_supported_id IS NULL THEN
        RAISE EXCEPTION 'No active routes found. Seed canonical transport data before stable demo data.';
    END IF;

    SELECT rs.stop_id INTO v_boarding_supported_id FROM route_stops rs WHERE rs.route_id = v_route_supported_id ORDER BY rs.stop_order ASC LIMIT 1;
    SELECT rs.stop_id INTO v_alighting_supported_id FROM route_stops rs WHERE rs.route_id = v_route_supported_id ORDER BY rs.stop_order DESC LIMIT 1;
    SELECT rs.stop_id INTO v_boarding_full_id FROM route_stops rs WHERE rs.route_id = v_route_full_id ORDER BY rs.stop_order ASC LIMIT 1;
    SELECT rs.stop_id INTO v_alighting_full_id FROM route_stops rs WHERE rs.route_id = v_route_full_id ORDER BY rs.stop_order DESC LIMIT 1;
    ...
    IF v_boarding_full_id IS NULL OR v_alighting_full_id IS NULL OR v_boarding_full_id = v_alighting_full_id THEN
        v_boarding_full_id := v_boarding_supported_id;
        v_alighting_full_id := v_alighting_supported_id;
        v_route_full_id := v_route_supported_id;
    END IF;
    ...
    WHERE university_id = v_supported_university_id AND route_id <> v_route_supported_id AND status = 'ACTIVE';
    ...
    VALUES ('SV-STABLE-MONTH', v_route_supported_id, EXTRACT(MONTH FROM CURRENT_DATE)::int, EXTRACT(YEAR FROM CURRENT_DATE)::int, date_trunc('month', CURRENT_DATE)::date, CURRENT_TIMESTAMP - INTERVAL '1 day', LEAST((date_trunc('month', CURRENT_DATE)::date + INTERVAL '1 month - 1 day')::date, v_end_date), 70000, 140000, 70000, 70000, v_policy_id, 'DEMO-MONTH-SV-STABLE-MONTH', 'ACTIVE')
    ...
    VALUES ('SV-STABLE-DAY', v_route_supported_id, v_boarding_supported_id, v_alighting_supported_id, 3500, 'DEMO-DAY-SV-STABLE-DAY', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '12 hours', 'UNUSED')
    ...
    VALUES ('SV-STABLE-DAY', 3500, 'BANK_TRANSFER', 'PAID', 'DEMO-PAY-DAY', CURRENT_TIMESTAMP, v_single_ticket_id, 'Thanh toán demo vé ngày')
    RETURNING payment_id INTO v_payment_id;

    INSERT INTO invoices (payment_id, student_code, description, amount, original_amount, subsidy_amount, final_amount, issued_at)
    VALUES (v_payment_id, 'SV-STABLE-DAY', 'Hóa đơn demo vé ngày', 3500, 7000, 3500, 3500, CURRENT_TIMESTAMP);
    ...
    VALUES ('SV-STABLE-HIST', v_route_supported_id, EXTRACT(MONTH FROM CURRENT_DATE)::int, EXTRACT(YEAR FROM CURRENT_DATE)::int, date_trunc('month', CURRENT_DATE)::date, CURRENT_TIMESTAMP - INTERVAL '3 days', LEAST((date_trunc('month', CURRENT_DATE)::date + INTERVAL '1 month - 1 day')::date, v_end_date), 70000, 140000, 70000, 70000, v_policy_id, 'DEMO-HIST-SV-STABLE-HIST', CURRENT_TIMESTAMP - INTERVAL '2 hours', 1, 'ACTIVE');
    ...
    INSERT INTO monthly_passes (student_code, route_id, effective_month, effective_year, valid_from, purchased_at, expires_on, fare_amount, original_fare_amount, subsidy_amount, final_fare_amount, subsidy_policy_id, qr_code, status)
    VALUES ('SV-STABLE-SUB', v_route_supported_id, EXTRACT(MONTH FROM CURRENT_DATE)::int, EXTRACT(YEAR FROM CURRENT_DATE)::int, date_trunc('month', CURRENT_DATE)::date, CURRENT_TIMESTAMP - INTERVAL '1 hour', LEAST((date_trunc('month', CURRENT_DATE)::date + INTERVAL '1 month - 1 day')::date, v_end_date), 70000, 140000, 70000, 70000, v_policy_id, 'DEMO-SUB-SV-STABLE-SUB', 'ACTIVE');

    INSERT INTO monthly_passes (student_code, route_id, effective_month, effective_year, valid_from, purchased_at, expires_on, fare_amount, original_fare_amount, subsidy_amount, final_fare_amount, subsidy_policy_id, qr_code, status)
    VALUES ('SV-STABLE-FULL', v_route_full_id, EXTRACT(MONTH FROM CURRENT_DATE)::int, EXTRACT(YEAR FROM CURRENT_DATE)::int, date_trunc('month', CURRENT_DATE)::date, CURRENT_TIMESTAMP - INTERVAL '1 hour', LEAST((date_trunc('month', CURRENT_DATE)::date + INTERVAL '1 month - 1 day')::date, v_end_date), 160000, 160000, 0, 160000, NULL, 'DEMO-FULL-SV-STABLE-FULL', 'ACTIVE');
    ...
    VALUES ('SV-STABLE-UNPAID', 'monthly', v_route_supported_id, 70000, 'Unpaid', 'Đơn demo vé tháng chưa thanh toán', NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'single-route', 'month', 'Điểm lên demo', 'Điểm xuống demo', 140000, 70000, 70000);

    INSERT INTO tb_orders (student_code, ticket_type, route_id, total, payment_status, name, paid_at, created_at, updated_at, order_mode, ticket_period, origin_label, destination_label, original_amount, subsidy_amount, final_amount)
    VALUES ('SV-STABLE-MONTH', 'monthly', v_route_supported_id, 70000, 'Paid', 'Đơn demo vé tháng đã thanh toán', CURRENT_TIMESTAMP - INTERVAL '1 day', CURRENT_TIMESTAMP - INTERVAL '1 day', CURRENT_TIMESTAMP, 'single-route', 'month', 'Điểm lên demo', 'Điểm xuống demo', 140000, 70000, 70000)
    ...
    VALUES (9000008001, 'SEPAY', CURRENT_TIMESTAMP - INTERVAL '1 day', 70000, 0, 70000, 'DEMOPAID', 'Giao dịch demo đã khớp', 'DEMO-REF-PAID', 'Nội dung giao dịch demo', '{"source":"STABLE_DEMO"}'::jsonb, v_order_id, CURRENT_TIMESTAMP - INTERVAL '1 day')
