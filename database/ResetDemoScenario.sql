...
-- Restore the cross-role practice flow without deleting unrelated demo history.
DO $scenario$
DECLARE
    ...
    v_old_schedule_ids integer[];
    v_depart_at timestamptz := date_trunc('minute', CURRENT_TIMESTAMP) + INTERVAL '20 minutes';
BEGIN
    SELECT u.user_id INTO STRICT v_student_user_id
    FROM users u
    WHERE u.email = 'student.supported@unibus.local';

    SELECT u.user_id INTO STRICT v_dispatcher_user_id
    FROM users u
    WHERE u.email = 'dispatcher.demo@unibus.local';

    SELECT r.route_id INTO v_route_id
    FROM routes r
    JOIN route_universities ru ON ru.route_id = r.route_id AND ru.status = 'ACTIVE'
    JOIN universities u ON u.university_id = ru.university_id AND u.code = 'DTU'
    WHERE r.status = 'ACTIVE'
      AND ru.active_from <= CURRENT_DATE
      AND (ru.active_until IS NULL OR ru.active_until >= CURRENT_DATE)
    ORDER BY CASE WHEN r.route_code = '16' THEN 0 ELSE 1 END, r.route_id
    LIMIT 1;

    IF v_route_id IS NULL THEN
        RAISE EXCEPTION 'Không tìm thấy tuyến trợ giá Duy Tân cho kịch bản xuyên role.';
    END IF;

    SELECT array_agg(DISTINCT t.schedule_id) FILTER (WHERE t.schedule_id IS NOT NULL)
    INTO v_old_schedule_ids
    FROM trips t
    WHERE t.notes = 'DEMO_SCENARIO:STUDENT_SUPPORTED';

    DELETE FROM vehicle_locations WHERE trip_id IN (SELECT trip_id FROM trips WHERE notes = 'DEMO_SCENARIO:STUDENT_SUPPORTED');
    DELETE FROM internal_messages WHERE trip_id IN (SELECT trip_id FROM trips WHERE notes = 'DEMO_SCENARIO:STUDENT_SUPPORTED');
    DELETE FROM incidents WHERE trip_id IN (SELECT trip_id FROM trips WHERE notes = 'DEMO_SCENARIO:STUDENT_SUPPORTED');
    DELETE FROM trips WHERE notes = 'DEMO_SCENARIO:STUDENT_SUPPORTED';
    DELETE FROM bus_schedules WHERE schedule_id = ANY(COALESCE(v_old_schedule_ids, ARRAY[]::integer[]));
    ...
    INSERT INTO bus_schedules (
        route_id, bus_id, driver_id, conductor_id, weekday_number,
        departure_time, end_time, status, assigned_by_user_id, assigned_at
    ) VALUES (
        v_route_id, NULL, NULL, NULL, EXTRACT(ISODOW FROM v_depart_at)::integer,
        v_depart_at::time, (v_depart_at + INTERVAL '90 minutes')::time,
        'ACTIVE', v_dispatcher_user_id, CURRENT_TIMESTAMP
    ) RETURNING schedule_id INTO v_schedule_id;

    INSERT INTO trips (
        schedule_id, route_id, bus_id, driver_id, conductor_id,
        service_date, departed_at, ended_at, status, notes
    ) VALUES (
        v_schedule_id, v_route_id, NULL, NULL, NULL,
        v_depart_at::date, NULL, NULL, 'NOT_STARTED', 'DEMO_SCENARIO:STUDENT_SUPPORTED'
    );
END
$scenario$;
