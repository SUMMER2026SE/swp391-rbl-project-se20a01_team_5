...
public class TravelHistoryService {
   ...
   public List<TravelHistoryRepository.TravelHistoryView> getHistory(CurrentUser currentUser, int page, int size) {
      if (page >= 0 && size >= 1 && size <= 100) {
         Student student = (Student)this.studentRepository.findByUserId(currentUser.userId()).orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "Student profile not found"));
         return this.travelHistoryRepository.findRecentByStudentCode(student.getStudentCode(), size, page * size);
      } else {
